...
import { EChartsOption, BarSeriesOption, XAXisComponentOption, YAXisComponentOption } from 'echarts';
import {
...
export default function transformProps(chartProps: EchartsBarChartProps): BarChartTransformedProps {
  ...
  const initBarSeries = (): BarSeriesOption[] =>{
    return [
      {
        type: 'bar',
        name: '辅助',
  ...
  const series: BarSeriesOption[] = initBarSeries();
